def dollar_amount(nickels, dimes, quarters, loonies, toonies):
    ...
    Return a str with the value of the coins nickels, dimes,
    quarters, loonies, and toonies in cents. The str must be of the
    form "# cents" where # is the total value of the coins.    
   ...
   ''' 
    ...
    loony = loonies * 100
    toony = toonies * 200
    dollar_amount = nickel + dime + quarter + loony + toony
    dollar_amount = max(dollar_amount, 0)    
    return str(str(dollar_amount) + ' cents')

               
def smallest_majority(group_size):
    '''(int) -> int

    Return the number of votes from a group of size group_size required
    for a strict majority. A strict majority occurs when more than
    half of a group votes in the same way.
    ...
    '''
    smallest_majority = group_size // 2
    smallest_majority = smallest_majority + 1

    return smallest_majority
...
def phone_bill(free_minutes, cents_per_minute, minutes_used):
    ...
    minute_used minutes are used and billed at cents_per_minute cents per
    minute with the first free_minutes minutes free.
    ...
    minutes = max(minutes_used - free_minutes, 0)
    payment = minutes * cents_per_minute
    phone_bill = payment / 100
